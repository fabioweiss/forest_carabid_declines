...
species_trends <- read.csv2("EWcarabids1999-2022_species_trends_traits.csv")
...
species_trends$forest_species_MUELLERMOTZFELD <- ordered(species_trends$forest_species_MUELLERMOTZFELD , levels = c("0", "U", "W", "F"))
species_trends$hibernation <- ordered(species_trends$hibernation , levels = c("Larva", "Larva/Imago", "Imago"))


species_trends$scaled_size <- as.numeric(scale(species_trends$size))
species_trends$scaled_latitude <- as.numeric(scale(species_trends$latitude))
species_trends$scaled_humidity_preference <- as.numeric(scale(species_trends$humidity_preference_SUSTEK))
species_trends$scaled_density_GAC <- as.numeric(scale(species_trends$density_GAC))
...
## trend vs. SPEI 
dat <- data.frame(table(species_trends$trend2, species_trends$spei_eff2))
...
ggplot(dat,
  ...
  geom_point(aes(x = Var2, 
                 y = Var1,
                 fill = Var1,
                 alpha = 0.05,
                 size = Freq), pch=21, color = "black" ) +
  ...
  geom_text(aes(label = Freq), 
            ...
            size = 10) +
  ggtitle("")+
  scale_x_discrete(position = "top", labels= c("positive","no effect","negative")) +
  scale_y_discrete(labels= c("decline","no trend","increase")) +
  scale_size_continuous(range = c(20, 40)) + # Adjust as required.
  scale_fill_manual(values = c(colors[6], colors[2], colors[2], colors[6],"lightgrey")) +
  labs(x = NULL, y = NULL) +
  theme(legend.position = "none",
        panel.background = element_blank(),
        panel.grid = element_blank(),
        axis.ticks = element_blank(),
        axis.text.x = element_text( hjust = -0.09),
...
plot2 <- ggplot(species_trends, aes(x = spei_eff2, y = size, fill = spei_eff2)) +
  ...
  ggtitle("")+
  geom_violin(trim=T, scale="width") +
  scale_x_discrete(labels= c("positive","no effect","negative"), position = "top") +
  geom_jitter(height=0.05, width=0.05, size=3) +
...
grid.arrange(plot1, plot2, widths=c(1.1,1), nrow=1)
...
grid.arrange(plot1, plot2, widths=c(1.2,1), nrow=1)
...
# local abundance 
plot1 <-  ggplot(species_trends, aes(x = trend2, y = corr_sum, fill = trend2)) +
  theme_dark() +
  theme(text = element_text(size = 30)) +
  labs(y = "", x = "")+
  ggtitle("")+
  geom_violin(scale = "width") +
  scale_x_discrete(labels= c("decline","no trend","increase")) +
...
plot2 <- ggplot(species_trends, aes(x = spei_eff2, y = corr_sum, fill = spei_eff2)) +
  theme_dark() +
  theme(text = element_text(size = 30)) +
  labs(y = "", x = "")+
  ggtitle("")+
  geom_violin(scale = "width") +
  scale_x_discrete(labels= c("positive","no effect","negative")) +
  scale_y_continuous(limits= c(0,6100), labels= c("low","high"), breaks = c(0,5800)) +
  geom_jitter(height=0.05, width=0.05, size=3) +
  scale_fill_manual(values = c(colors[2], colors[6],colors[8]))+
  theme(legend.position = "none",
